...
class Car:
    ...
    def walk_turn(self, walk_distance, turn_angle, left_speed = '3.0', right_speed = '3.0'):#小车行走距离。参数：行驶的距离，（左右轮的速度）
        self.left_speed, self.right_speed = left_speed, right_speed
        if walk_distance != 0 and turn_angle == 0:
            serialHandle.write((str(self.left_speed) + ';' + str(self.right_speed) + ';' + str(turn_angle)+';0.0').encode('utf-8'))
            time.sleep(walk_distance // left_speed)
            serialHandle.write(('0.0' + ';' + '0.0' + ';' + '0.0'+';0.0').encode('utf-8'))
        elif walk_distance == 0 and turn_angle != 0:
            if turn_angle > 0:
                serialHandle.write((str(self.left_speed) + ';' + '-' + self.right_speed + ';' + str(turn_angle)+';0.0').encode('utf-8'))
                print(str(self.left_speed) + ';' + '-' + self.right_speed + ';' + str(turn_angle))
            elif turn_angle < 0:
                serialHandle.write(('-' + self.left_speed + ';' + str(self.right_speed) + ';' + str(math.fabs(turn_angle))+';0.0').encode('utf-8'))
                print('-' + self.left_speed + ';' + str(self.right_speed) + ';' + str(turn_angle)+';0.0')
        elif walk_distance != 0 and turn_angle != 0:
            print('实现不了')
            exit()
    ...
    def walk(self,dis):
        send_msgs = serialHandle.write(('3.0;3.0;0.0;'+str(dis)).encode('utf-8'))
        print ('3.0;3.0;0.0;'+str(dis))

    def auto_walk(self, left_speed, right_speed, turn_angle = 0.0):
        serialHandle.write((str(left_speed) + ';' + str(right_speed) + ';' + str(turn_angle)+';0.0').encode('utf-8'))
        print(str(left_speed) + ';' + str(right_speed) + ';' + str(turn_angle)+';0.0')
...
class Camera:# 2 - 12 右 - 左 上 - 下
    def __init__(self) -> None:
        ...
        makerobo_pins = [3, 2]  # PIN管脚字典  gpio.BOARD对应实际物理管脚  gpio.BCM对应BCM编码
        ...
        self.LR_pin.start(4.5)
        self.HL_pin.start(7)
    
    def turn(self, LR_angle, HL_angle):#控制相机云台旋转。参数：左右旋转角度，上下旋转角度
        
        self.LR_pin.ChangeDutyCycle(0.05555 * LR_angle + 2)  # 改变占空比
        self.LR_angle = LR_angle
        self.HL_pin.ChangeDutyCycle(0.05555 * HL_angle + 2)
        self.HL_angle = HL_angle
            
    def follow_fruit(self, recvData):#控制相机，使识别到的果子一直在相机的中心位置
        if recvData[0] > pic_xy[0] // 2:   #pic_xy = [640, 480]            
            self.turn(self.LR_angle - 0.2, self.HL_angle)
            self.LR_angle -= 0.2
            #time.sleep(0.2)
        elif recvData[0] < pic_xy[0] // 2:
            self.turn(self.LR_angle + 0.2, self.HL_angle)
            self.LR_angle += 0.2
            #time.sleep(0.2)

        if recvData[1] > pic_xy[1] // 2:
            self.turn(self.LR_angle, self.HL_angle + 0.2)
            self.HL_angle += 0.2
            #time.sleep(0.2)
        elif recvData[1] < pic_xy[1] // 2:
            self.turn(self.LR_angle, self.HL_angle - 0.2)
            self.HL_angle -= 0.2
            #time.sleep(0.2)
...
def hard_to_app():
    #需要判断格式
    while True:
        receive_data = serialHandle.readline()
        #print(receive_data)
        global ser_data
        ser_data = receive_data.decode()
...
def depth_get_process():
    Depth = subprocess.getoutput('echo $(./a)')
...
def camera_LR_HL_move(my_camera):
    while True:
        recvData = client.makefile().readline()#接收
...
def auto_grab_fruit():#18.9 - 16.9, 自动流程
    ...
    while fruit_arrange:
        ...
        my_camera.turn(45, 90)#相机转向，并且执行寻找水果
        print('turn', my_camera.get_angle())
        time.sleep(1)
        my_car.auto_walk(3.0, 3.0)
        ...
        while True:
            ...
            print(recvData)
            my_camera.follow_fruit(recvData)
            print('follow', my_camera.get_angle())
        camera_angel = int(my_camera.get_angle()[0]) #计算距离
        depth = float(depth_get_process())
        print(camera_angel)
        car_distance_front = depth * math.sin(math.radians(90 - int(camera_angel)))
        car_distance_right = depth * math.cos(math.radians(90 - int(camera_angel)))
        
        fruit_number = init_number_fruit
        
        while fruit_number:
            camera_control = threading.Thread(name='python_camera_move', target= camera_LR_HL_move, args=(my_camera,))#将服务器消息发给32
            camera_control.start()#靠近果实，并一直追随
            my_car.walk(20.0)#需要时延
            print('walk start')
            while('S' not in ser_data):
                pass
            print('car stop')
            ...
            my_car.walk_turn(0, -90.0)
            time.sleep(5)
            camera_angel = my_camera.get_angle() 
            fruit_Z = depth_get_process()#获取三位坐标值
            fruit_X = pic_xy[0] / 2
            fruit_Y = fruit_Z*math.sin(camera[0]*math.pi/180)
            print(fruit_X,fruit_Z,fruit_Y)
...
if __name__ == '__main__':
    camera_control = threading.Thread(name='hard_send', target= hard_to_app, args=())#send 32 data to server
